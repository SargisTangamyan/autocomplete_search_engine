...
AutocompleteEngine::query(const std::string& prefix) {
...
std::vector<Completion>
AutocompleteEngine::fuzzy_query(const std::string& prefix) {
    std::string norm = normalize(prefix);
    if (!norm.empty()) cache_.record(norm);
